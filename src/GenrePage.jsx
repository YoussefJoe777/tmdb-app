...
import "./pages/Movies/Movies.css"; // نفس الستايل الموحد

const GenrePage = () => {
  const { id } = useParams(); // ID الخاص بالـ genre
  ...
  const fetchMovies = async (pageNum) => {
    ...
    const url = `https://api.themoviedb.org/3/discover/movie?with_genres=${id}&language=en-US&page=${pageNum}`;
    const options = {
      method: "GET",
      headers: {
  ...
  // أول مرة أو عند تغيير الـ genre
  useEffect(() => {
    setMovies([]);
    setPage(1);
    setHasMore(true);
  ...
  return (
    ...
    >
      <h1 className="text-white text-2xl font-bold mb-4">Genre Movies</h1>
      <div
        style={{
          display: "grid",
      ...
      >
        {movies.map((movie) => (
          ...
          >
            ...
            <p className={`movie-overview ${expanded[movie.id] ? "expanded" : ""}`}>
              {movie.overview}
            </p>
